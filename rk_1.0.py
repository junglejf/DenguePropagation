...
#           y[0] ,    y[1]  ,  y[2]    ,  y[3]     ,    y[4]   , y[5]      ,  y[6]    ,   y[7]   ,  y[8]    ,  y[9]                
    ...
    ## Humanos ##
    #f7 = coef[19] * N - (coef[14] * (y[5]  / y[6] ) + coef[18]) * y[6]        # Página 45, equação s(t)
    ...
    #for i in range (0,len(yFa)):
        #if not(precisao_numerica(yFa[i])):
           # print(yFa[i])
            #yFa[i] = y[i]
...
def desenha_grafico (titulo,eixoX, eixo, nome_eixoX, nome_eixoY, nome_das_curvas,figura = plt.figure(),eixoYadicional = None, eixoXadicional = None):
    grafico = figura
    plt.title(titulo)
    ...
    else: # se for somente uma variável em y (x,y)
...
print('h = '+str(h)+' com '+str(dias)+' dias simulados, ci = '+str(Ci)+', tinicial = '+str(t))
print("Y0: ")
print(Y0)
print("Y1: ")
print(Y1)
...
eixoY = filtrar_variavel(YM,filtro) # <--- W(t)
eixoY2 = filtrar_variavel(YN,filtro)
eixoYcomEsemcontrole = []
eixoYcomEsemcontrole.append(eixoY)
eixoYcomEsemcontrole.append(eixoY2)
...
#nome_das_curvas = ['ovo','larva','pupa','Mosquito suscetivel(W1)','Mosquito exposto(W2)','Mosquito infectado(W3)','st','et','it','rt']
eixo = eixos(YN, filtro)
grafico = plt.figure()
desenha_grafico (titulo,TM, eixoY, nome_eixoX, nome_eixoY, nome_das_curvas[3:],grafico,eixoY2,TN)
plt.legend(loc='upper center', bbox_to_anchor = (1.38,0.9),shadow = True)
plt.show()